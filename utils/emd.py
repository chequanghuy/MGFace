...
def patch_similarity(anchor_patch, gallery_patch, method='mean', temperature=1.0):
    """
    anchor_patch:  (C, R)
    gallery_patch: (N, C, R)
    ...
    return:
        sim: (N,)
    """
    sim_patch = torch.einsum('cr,ncr->nr', anchor_patch, gallery_patch)  # (N, R)

    if method == 'mean':
        sim = sim_patch.mean(dim=1)
