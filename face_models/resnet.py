...
class ResNetFace(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("88",embedding_88.size(),x.size())
        embedding_44 = None
        if self.pooling:
...
class ResNetFaceMask(nn.Module):
    def __init__(self, block, layers, use_se=True, use_reduce_pool=False,num_mask_classes=2, dropout_p=0.2):
        self.inplanes = 64
        self.use_se = use_se
        super(ResNetFaceMask, self).__init__()
        self.conv1 = nn.Conv2d(1, 64, kernel_size=3, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.prelu = nn.PReLU()
        self.maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        embedding_88 = x
        embedding_44 = torch.nn.functional.adaptive_avg_pool2d(x, output_size=(4, 4))
        logit_mask = self.mask_head(embedding_44)
...
def resnet_face18(use_se=True, **kwargs):
    ...
    return model
def resnet_face18_mask(use_se=True, **kwargs):
    model = ResNetFaceMask(IRBlock, [2, 2, 2, 2], use_se=use_se, **kwargs)
    return model